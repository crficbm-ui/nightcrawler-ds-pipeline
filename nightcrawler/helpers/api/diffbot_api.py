...
class DiffbotAPI(APICaller):
    def __init__(self, context, cache_name="diffbot", max_retries=3, retry_delay=2):
    ...
    def call_api(self, url, config=DEFAULT_CONFIG, force_refresh=False):
        ...
        if not force_refresh and (cached := self._read_cache(data_hash)) is not None:
            logger.warning("Using cached response for diffbot (%s)", data_hash)
            return cached
        ...
        raise Exception("All API call attempts to diffbot failed.")
